...
out={'generated_at':now.isoformat(),'edition':edition,'stories':chosen}; name=f"{now:%Y-%m-%d}_{edition}.json";(DATA/name).write_text(json.dumps(out,indent=2));print(name,len(chosen));send_newsletter(out)
